...
| VFS library image | ✅ 65.3 MB raw / 30.2 MB gz, 1 571 files, 67 packages, mounts in 0.3 s (was 213.9 MB / 46.6 MB / 17 251 before `include,lib` stripping) |
...
30.2 MB gzipped against 54.7 MB, i.e. about 45% less to download.
...
packages exist for, and nothing in a browser compiles: measured on the 213.9 MB
image they were 148.6 MB of it — 128.0 MB Boost headers from `BH`, 8.8 MB
`RcppEigen`, 7.6 MB `StanHeaders`. The rebuilt image is **65.3 MB unpacked and
30.2 MB gzipped across 1 571 files**, still all 67 packages: 69% off what the
browser holds in memory and 35% off the download, and it mounts in 302 ms
rather than 1 659 ms. `stringi` is now the largest single item at 35.0 MB and
is genuinely required.
`StanHeaders/stanc.js` must *not* be removed: `rstan:::.onLoad()` sources it
with `mustWork = TRUE`. It is a file, and `strip` removes only directories, so
it survives. See the how-to, §7.
...
`make webr-verify` runs exactly that command for you, doing the npm setup
itself — it is the same script the GitHub `verify` job runs, so a failure
there is a failure in CI. For a readable fit rather than the CI assertions,
use `make webr-demo` (see [End-to-end check](#end-to-end-check)).
...
stamp, plus a `build/webr/.image-pins` sentinel recording `WEBR_TAG`,
`EMSDK_VERSION`, `R_VERSION` and `RWASM_REF`, so `docker compose build` is
re-invoked when a copied file or a toolchain pin changes -- a re-run with
nothing changed skips straight to the compile step. (The sentinel is needed
because the pins are build args, not files, and Make has no memory of a
variable's previous value.) Force a rebuild with
...
RBEST_DEPS=false RBEST_VARIANTS=notbb \
  docker compose run --rm build                    # today a synonym of baseline
```
...
make webr-verify          # the CI assertions: patch markers, parser, gMAP fit
```
...
- **`rwasm` caches the source tarball.** `rwasm:::update_repo()` skips
  ...
  iterations.
- **`configure` regenerates `src/Makevars`.** A package whose rstantools
  `configure` runs `rstantools::rstan_config()` at install time undoes any edit
  to `src/Makevars`. The fix is `src/Makevars.webr`, rwasm's supported override
  hook: it copies the file over `src/Makevars` *and* passes `--no-configure`.
  `stage_source()` writes one for every RBesT variant (and
  `patch-rstan-tarball.R` one for rstan), having already run `rstan_config()`
  on the staged tree itself. That is also what lets RBesT ship the stock
  rstantools `configure`, with no WebAssembly logic in the released sources.
...
The mount figures are from the stripped image; the posterior comparison
predates stripping and is unaffected by it.

```
webR ready                    1.1 s
VFS mount (65.3 MB, 1 571 files)   0.3 s
