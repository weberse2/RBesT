...
##     opt-in, to be enabled *after* a failure has been classified. Note that
##     `notbb` is today a synonym of `baseline`: removing the native
##     RcppParallel/TBB linkage is unconditional (see stage_source()), because
##     it moved here out of the wasm branch of RBesT's `configure`, which is
##     now the stock rstantools script. The name is kept because the Makefile,
##     the workflow and section 9 of the design note refer to it, and because
##     it is where a genuinely TBB-less variant would go if one is ever needed
##     again.
##
...
##                   (default: baseline; notbb == baseline, see above)
...
## never satisfy another's undefined symbols. Both copies come from the same
## file -- tools/webr/tbb-stubs.cpp, which stage_source() below injects into
## RBesT's staged src/ and patch-rstan-tarball.R into rstan's tarball. See
...
stage_source <- function(variant) {
  ...
  ## RTLD_LOCAL every module has to define what it imports. This is the only
  ## mechanism that puts them there: the file is a build-time input of this
  ## route (tools/ is .Rbuildignore'd) and the released package neither ships
  ## it nor contains anything that would compile it. src/ has just been
  ## regenerated by the rstan_config() call above, so nothing hand-written
  ## survives there and the copy has to happen here, after it.
  ...
  ## Remove the RcppParallel/TBB linkage from the rstantools Makevars, and pin
  ## the object list -- for every variant, unconditionally.
  ##
  ...
  ##
  ## Both halves used to be done by the wasm branch of RBesT's `configure`
  ## instead. They moved here so that the released `configure` is the stock
  ## rstantools one and carries no WebAssembly logic at all -- none of it has
  ## any business being shipped to CRAN, and this route never needed it: it
  ## stages the source tree itself.
  ...
  ## Pin the object list. R CMD INSTALL globs src/ for sources and passes the
  ## result to make as `OBJECTS=...`, which would already pick up
  ## webr-tbb-stubs.o -- the generated src/Makevars sets no OBJECTS of its own.
  ## The wasm build is the one place where being wrong about that is invisible:
  ## the module links, installs and ships, and only fails when webR dlopen()s
  ## it. R defers to an OBJECTS defined in Makevars (it passes its own list
  ## only when the file matches no `^OBJECTS *=`), so from here on the list
  ## below is what gets linked, and it is built from the files that are
  ## actually present rather than hard-coded. This is the same remedy
  ## patch-rstan-tarball.R names for rstan's side of the problem.
  objects <- character()
  for (source in list.files(file.path(dest, "src"))) {
    if (grepl("[.](c|cc|cpp)$", source)) {
      objects <- c(objects, sub("[.][^.]*$", ".o", source))
    } else if (grepl("[.](f|f90|f95|m|mm)$", source)) {
      ## R would compile these too. Rather than drop them silently and fail at
      ## link time with a missing symbol, say so here.
      stop(
        "unexpected source src/", source, " in the generated src/; ",
        "extend the OBJECTS list in stage_source() to cover it"
      )
    }
  }
  if (!"webr-tbb-stubs.o" %in% objects) {
    stop("webr-tbb-stubs.o is not in the wasm object list")
  }
  mk <- c(mk, paste("OBJECTS =", paste(objects, collapse = " ")))
  ...
  ## file and put the TBB flags back. The rstan_config() call above has
  ## already generated everything `configure` would have.
  writeLines(mk, file.path(dest, "src", "Makevars.webr"))
  message(
    "staged src/Makevars.webr for '", variant, "':\n",
    paste0("  ", grep("^(PKG_|OBJECTS)", mk, value = TRUE), collapse = "\n")
  )
...
if (!any(results$status == "SUCCESS")) {
  message(
    ...
    "                                  PKG_LIBS is already blanked for every\n",
    "                                  variant, so look at src/Makevars.webr\n",
...
## 148.6 MB of the 213.9 MB image, 128 MB of it Boost headers from BH, which is a
...
if (!identical(Sys.getenv("RBEST_VFS", "true"), "false")) {
  ...
  ## The image *is* the deliverable, so a failure here has to be fatal. It is
  ## not enough to report it: the patch manifest below would otherwise still
  ## (re)create `vfs/` and write PATCHES.txt into it, leaving a non-empty
  ## directory with no .data/.js.metadata in it. Every downstream consumer then
  ## reports the wrong thing -- CI's `if-no-files-found: error` upload sees
  ## files and passes, SHA256SUMS checksums the manifest alone, and the failure
  ## first surfaces a job later as `ls: image/*.js.metadata: No such file`,
  ## with the real error left behind in the build log.
  if (!identical(status, "SUCCESS")) quit(status = 1L)
