...
## `configure` (`rstantools::rstan_config()`) is not just about generating
## `src/Makevars`: it is also what generates the Stan model C++ sources
...
## which does the same regeneration for local dev builds). `rwasm` skips
## `./configure` for wasm builds (its own substitute would blow away any
## Makevars override anyway), so without running it ourselves first RBesT
## would have no C++ sources to compile at all; `R CMD INSTALL` treats
## compiling zero files as trivially successful, silently producing a package
## with no `RBesT.so`, which then fails to load.
##
## Run that regeneration here, before `local::.` is packaged, using a
## host-native rstan/StanHeaders (needed to transpile `.stan` to C++, but
...
## The `src/Makevars` just regenerated queries the host's native
## RcppParallel for its linker flags via `RcppParallel::RcppParallelLibs()`,
## which resolve to the host's native `libtbb.so`/`libtbbmalloc.so` --
## wasm-ld cannot link those into a wasm binary:
##
##   wasm-ld: error: unknown file type: .../RcppParallel/lib/libtbb.so
##
## The wasm build of RcppParallel does not use TBB at all (it falls back to a
## header-only tinythread implementation), so the correct wasm linker flags
## are simply none. `rwasm` has a `src/Makevars.webr` sibling-file override
## mechanism for exactly this kind of case, but `local::.` packages are
## copied into the build verbatim by `rwasm` (bypassing `R CMD build`/
## `.Rbuildignore`), so relying on a separate override file being picked up
## at the right moment is fragile. Patch the regenerated `src/Makevars`
## in place instead: it is never committed either way (entirely generated by
## `configure`, gitignored), so rewriting it here for this build only is
## harmless.
mkvars <- readLines("src/Makevars")
pkg_libs <- grep("^PKG_LIBS", mkvars)
stopifnot(length(pkg_libs) == 1)
mkvars[pkg_libs] <- sub(
  '\\$\\(shell "\\$\\(R_HOME\\)/bin\\$\\(R_ARCH_BIN\\)/Rscript" -e "RcppParallel::RcppParallelLibs\\(\\)"\\)\\s*',
  "",
  mkvars[pkg_libs]
)
writeLines(mkvars, "src/Makevars")
