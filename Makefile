...
# rebuild -- when one of those files changes. To force a rebuild regardless,
# `rm build/webr/.image-built` or `docker compose build --no-cache` directly
# in $(WEBR_DOCKER_DIR).
#
# The toolchain pins are build args rather than files, and Make has no memory
# of a variable's previous value, so a stamp keyed on the COPYed files alone
# would survive a pin bump and silently reuse the old toolchain. .image-pins
# records them so a change invalidates the stamp. Make imports the environment,
# so `EMSDK_VERSION=5.1.0 make r-binary-webr` is seen here; unset simply means
# "whatever compose.yaml defaults to", and compose.yaml is itself a
# prerequisite, so a change to those defaults rebuilds too.
WEBR_PINS := WEBR_TAG=$(WEBR_TAG) EMSDK_VERSION=$(EMSDK_VERSION) \
  R_VERSION=$(R_VERSION) RWASM_REF=$(RWASM_REF)
...
build/webr/.image-pins : FORCE
	@install -d build/webr
	@printf '%s\n' '$(WEBR_PINS)' | cmp -s - $@ 2>/dev/null || \
	  printf '%s\n' '$(WEBR_PINS)' > $@

build/webr/.image-built : $(WEBR_DOCKER_FILES) build/webr/.image-pins | webr-docker-ready
	install -d build/webr
	cd $(WEBR_DOCKER_DIR); \
