...
#' @param weight weight given to the non-informative component (0 < `weight` < 1).
...
#' Hypotheses and its Relationship to the Schwarz Criterion *J
...
#' practical applications, `RBesT` uses `n+1` as the sample
#' size such that the default robust prior is the uniform instead of
#' the `Beta(1/2,1/2)` which strictly defined would be the unit
...
#' @describeIn robustify The default `mean` is set to the mean of the
...
robustify.gammaMix <- function(priormix, weight, mean, n = 1, ...) {
  ...
  if (missing(mean)) {
    s <- summary(priormix)
    message(paste(
      "Using default mean for robust component; the mean of the prior which is",
      s["mean"],
      "."
    ))
    mean <- s["mean"]
  ...
  rob <- mixgamma(
    robust = c(1, mean, n),
    param = "mn",
    likelihood = likelihood(priormix)
  )
  mixcombine(priormix, rob, weight = c(1 - weight, weight))
}

#' @describeIn robustify The default `mean` is set to the mean
...
#' sampling standard deviation using the `sigma` argument.
