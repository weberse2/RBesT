...
historical data on variances has been suggested in @schmidli2017. The utility is
...
it's moment and variance can analytically be shown to be (see the
[Wikipedia article on the Gamma distribution](https://en.wikipedia.org/wiki/Gamma_distribution#Logarithmic_expectation_and_variance), for
example)
...
In the following we reanalyze the main example of @schmidli2017 which
is shown in table 2:
...
parameter $\tau$ and the intercept $\beta$ is needed. In @gelman2014
...
standard deviation of $100$ which is in line with @schmidli2017:
...
In @schmidli2017 the correct $\Gamma$ likelihood is used in contrast
