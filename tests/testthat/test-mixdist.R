...
mixdist_tolerance_lower <- function() 1e-5

mixdist_tolerance <- function() 1E-2

mixdist_quantile_sample_size <- function() 3

mixdist_large_sample_size <- function() 1E6

mixdist_case <- function(name) {
  switch(
    name,
...
pmix_lower_tail_test <- function(mix, N = mixdist_quantile_sample_size()) {
  ## sample some random quantiles
  do_test <- function(mix) {
    withr::local_seed(234534)
    q <- rmix(mix, N)
    pl <- pmix(mix, q, lower.tail = TRUE)
    pu <- pmix(mix, q, lower.tail = FALSE)
    res <- abs(pl - (1 - pu))
    expect_true(all(res < mixdist_tolerance_lower()))
  }
...
test_that("Beta quantile function is correct", {
  mix_simul_test(mixdist_case("beta"), mixdist_tolerance(), c(0.1, 0.9))
})
test_that("Beta mixture quantile function is correct", {
  mix_simul_test(mixdist_case("betaMix"), mixdist_tolerance(), c(0.1, 0.9))
})

test_that("Normal quantile function is correct", {
  mix_simul_test(mixdist_case("norm"), mixdist_tolerance(), c(-1, 0))
})
test_that("Normal mixture quantile function is correct", {
  mix_simul_test(mixdist_case("normMix"), mixdist_tolerance(), c(4, 1), seed = 234535)
})
test_that("Normal mixture with very weak component quantile function is correct", {
  mix_simul_test(mixdist_case("normMixWeak"), mixdist_tolerance(), c(4, 1), seed = 234535)
})

test_that("Gamma quantile function is correct", {
  mix_simul_test(mixdist_case("gamma"), mixdist_tolerance(), c(2, 7))
})
test_that("Gamma mixture quantile function is correct", {
  mix_simul_test(mixdist_case("gammaMix"), mixdist_tolerance(), c(2, 7), ptest = seq(0.2, 0.8, by = 0.1))
})

test_that("Singular gamma mixture quantile function is correct", {
  mix_simul_test(
    mixdist_case("gammaMix2"),
    10 * mixdist_tolerance(),
    c(1, 1E3),
...
test_that("Beta CDF function is consistent", {
  consistent_cdf(mixdist_case("beta"), seq(0.1, 0.9, by = 0.1))
})
test_that("Beta mixture CDF function is consistent", {
  consistent_cdf(mixdist_case("betaMix"), seq(0.1, 0.9, by = 0.1))
})

test_that("Normal CDF is consistent", {
  consistent_cdf(mixdist_case("norm"), seq(-2, 2, by = 0.1))
})
...
test_that("Gamma CDF function is consistent", {
  consistent_cdf(mixdist_case("gamma"), seq(2, 7, by = 0.1))
})
test_that("Gamma mixture CDF function is consistent", {
  consistent_cdf(mixdist_case("gammaMix"), seq(2, 7, by = 0.1))
})
...
test_that("Problematic (1) BetaBinomial CDF function is consistent", {
  consistent_cdf(preddist(mixdist_case("bm1"), n = 50), 0:50)
})
test_that("Problematic (1) BetaBinomial CCDF function is consistent", {
  consistent_ccdf(preddist(mixdist_case("bm1"), n = 50), 0:50)
})
test_that("Problematic (2) BetaBinomial CDF function is consistent", {
  consistent_cdf(preddist(mixdist_case("bm2"), n = 50), 0:50)
})
test_that("Problematic (2) BetaBinomial CCDF function is consistent", {
  consistent_ccdf(preddist(mixdist_case("bm2"), n = 50), 0:50)
})
...
test_that("Multivariate normal mixture has consistent initialization", {
  ...
  expect_equal(summary(mv1)$cov, S, tolerance = mixdist_tolerance_lower())
  expect_equal(summary(mv2)$cov, S, tolerance = mixdist_tolerance_lower())
  expect_equal(summary(mv3)$cov, S / 2, tolerance = mixdist_tolerance_lower())
})

mvn_consistent_summaries <- function(mix, S = mixdist_large_sample_size()) {
  samp <- rmix(mix, S)
  m <- colMeans(samp)
  expect_equal(colMeans(samp), summary(mix)$mean, tolerance = mixdist_tolerance())
  expect_equal(cov(samp), summary(mix)$cov, tolerance = mixdist_tolerance())
}
