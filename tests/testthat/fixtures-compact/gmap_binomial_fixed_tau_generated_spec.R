gmap_binomial_fixed_tau_generated_compact_spec <- 
structure(list(name = "gmap_binomial_fixed_tau_generated", seed = 873461L, 
    nc = 1L, has_intercept = TRUE, tau_fixed = TRUE, tau_values = c(`tau[1]` = 0.5), 
    ...
    "theta[2]", "theta[3]", "theta[4]", "theta[5]", "theta[6]", 
    "theta[7]", "theta[8]", "tau[1]", "beta[1]", "theta_pred", 
    ...
    "n_leapfrog__", "divergent__", "energy__"), metadata_mcmc = list(
        ...
        recipe = "tests/testthat/fixtures-compact-src/gmap_binomial_fixed_tau_generated_fixture.R", 
        nc = 1L, digits = 4L, seed = 873461L, data_objects = character(0))), class = "compact_gmap_fixture_spec")

gmap_binomial_fixed_tau_generated_compact_spec$mvn_model <- read_mix_json(
  testthat::test_path("fixtures-compact", "gmap_binomial_fixed_tau_generated_mvn_model.json"),
  rescale = TRUE
)
gmap_binomial_fixed_tau_generated_compact_spec$mvn_theta <- read_mix_json(
  testthat::test_path("fixtures-compact", "gmap_binomial_fixed_tau_generated_mvn_theta.json"),
  rescale = TRUE
)
gmap_binomial_fixed_tau_generated_compact_spec$builder <- function() {
  withr::with_options(
